...
<div class="p-4 sm:ml-60 min-h-screen">
    <div class="p-2">
        <h1 class="text-2xl font-semibold mt-10 mb-4">Data Ekstrakurikuler</h1>

        <a href="{{ route('ekstrakurikuler.create') }}" class="bg-green-600 text-white py-2 px-4 rounded-lg hover:bg-green-700 mb-3 inline-block">Create New Ekstrakurikuler</a>
                ...
                <tbody>
                    @foreach($ekstrakurikulers as $ekstrakurikuler)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="py-3 px-4 border-b text-center">{{ $ekstrakurikuler->nama }}</td>
                            <td class="py-3 px-4 border-b text-center">{{ $ekstrakurikuler->deskripsi }}</td>
                            <td class="py-3 px-4 border-b text-center">
                                <div class="flex justify-center space-x-2">
                                    <a href="{{ route('ekstrakurikuler.edit', $ekstrakurikuler->ekstrakurikulerID) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg p-2" title="Edit">
                                    ...
                                    <form action="{{ route('ekstrakurikuler.destroy', $ekstrakurikuler->ekstrakurikulerID) }}" method="POST" class="inline" title="Hapus">
                                        @csrf
                                        @method('DELETE')
