...
<div class="p-4 sm:ml-60 min-h-screen">
    <div class="p-2">
        <h1 class="text-2xl font-semibold mt-10 mb-4">Data Ekstrakurikuler Siswa</h1>

        <a href="{{ route('siswa.ekstrakurikuler_siswa.create', $siswa->siswaID) }}" class="bg-green-600 text-white py-2 px-4 rounded-lg hover:bg-green-700 mb-3 inline-block">Tambah Data</a>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full bg-white border border-gray-200 rounded-lg shadow-md">
                ...
                <tbody>
                    @foreach($siswaEkstrakurikuler as $item)
                        <tr class="hover:bg-gray-50">
                            <td class="py-3 px-4 border-b text-center">{{ $loop->iteration }}</td>
                            <td class="py-3 px-4 border-b text-center">{{ $item->ekstrakurikuler->nama ?? 'Data tidak ditemukan' }}</td>
                            ...
                            <td class="py-3 px-4 border-b text-center">
                                <div class="flex justify-center space-x-2">
                                    <a href="{{ route('siswa.ekstrakurikuler_siswa.edit', [$siswa->siswaID, $item->siswaEkstrakurikulerID]) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg p-2" title="Edit">
                                    ...
                                    <form action="{{ route('siswa.ekstrakurikuler_siswa.destroy', [$siswa->siswaID, $item->siswaEkstrakurikulerID]) }}" method="POST" class="inline" title="Hapus">
                                        @csrf
                                        @method('DELETE')
