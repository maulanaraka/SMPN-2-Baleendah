...
<div class="p-4 sm:ml-60 min-h-screen">
    <div class="p-2">
        <h1 class="text-2xl font-semibold mt-10 mb-4">Data Mata Pelajaran</h1>

        <a href="{{ route('mata_pelajaran.create') }}" class="bg-green-600 text-white py-2 px-4 rounded-lg hover:bg-green-700 mb-3 inline-block">Create New Mata Pelajaran</a>
                ...
                <tbody>
                    @foreach($mataPelajarans as $mataPelajaran)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="py-3 px-4 border-b text-center">{{ $mataPelajaran->mataPelajaran }}</td>
                            ...
                            <td class="py-3 px-4 border-b text-center">
                                <div class="flex justify-center space-x-2">
                                    ...
                                    <form action="{{ route('mata_pelajaran.destroy', $mataPelajaran->mataPelajaranID) }}" method="POST" class="inline" title="Hapus">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" onclick="return confirm('Apakah Anda yakin ingin menghapus mata pelajaran ini?')" class="bg-red-500 hover:bg-red-600 text-white rounded-lg p-2">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5">
