...
<div class="p-4 sm:ml-60 min-h-screen">
    <div class="p-2">
        <h1 class="text-2xl font-semibold mt-10 mb-4">Data Prestasi</h1>

        <a href="{{ route('prestasi.create') }}" class="bg-green-600 text-white py-2 px-4 rounded-lg hover:bg-green-700 mb-3 inline-block">Tambah Prestasi</a>
            ...
            <table class="min-w-full bg-white border border-gray-200 rounded-lg shadow-md">
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="py-3 px-4 border-b text-center">{{ $prestasi->bidang }}</td>
                        <td class="py-3 px-4 border-b text-center">{{ $prestasi->tanggal }}</td>
                        <td class="py-3 px-4 border-b text-center">{{ $prestasi->keteranganPrestasi }}</td>
                        <td class="py-3 px-4 border-b text-center">
                            <div class="flex justify-center space-x-2">
                                <a href="{{ route('prestasi.edit', $prestasi->prestasiID) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg p-2" title="Edit">
                                ...
                                <form action="{{ route('prestasi.destroy', $prestasi->prestasiID) }}" method="POST" class="inline" title="Hapus">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" onclick="return confirm('Apakah Anda yakin ingin menghapus?')" class="bg-red-500 hover:bg-red-600 text-white rounded-lg p-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5">
