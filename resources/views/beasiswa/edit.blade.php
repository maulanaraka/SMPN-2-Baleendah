...
<div class="p-4 sm:ml-60 min-h-screen">
    <div class="p-2">
        ...
        <!-- Edit Beasiswa -->
        <form action="{{ route('beasiswa.update', $beasiswa->beasiswaID) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div class="col-span-1">
                    ...
                    <input type="text" name="penyelenggara" id="penyelenggara" class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="{{ $beasiswa->penyelenggara }}" required>
                </div>

                <div class="col-span-1">
                    <label for="tahun" class="block mb-2 text-sm font-medium text-gray-900">Tahun</label>
                    <input type="number" name="tahun" id="tahun" class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="{{ $beasiswa->tahun }}" required>
                </div>
            </div>

            <div class="mt-6 flex justify-between">
                <button type="submit" class="bg-green-600 text-white py-2 px-4 rounded-lg hover:bg-green-700">Update</button>
                <a href="{{ route('beasiswa.index') }}" class="bg-red-600 text-white py-2 px-4 rounded-lg hover:bg-red-700">
                    Cancel
                </a>
            </div>
        </form>
